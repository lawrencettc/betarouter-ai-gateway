...
export async function generateMetadata({
	...
	const title = `The ${year} BetaPass Model Census — coding models rated by the developers who pay for them`;
	const description = `Value, quality, and speed scores for coding models, rated only by BetaPass developers with verified real-world usage. No benchmarks, no vibes — shipped-code verdicts.`;
	return {
		title,
		description,
		...
		twitter: {
			...
			title: `The ${year} BetaPass Model Census`,
			description,
		},
	};
}
...
export default async function CensusPage({
	...
	const jsonLd = {
		...
		"@type": "Dataset",
		name: `The ${year} BetaPass Model Census`,
		description:
			"Coding LLMs rated on value for money, output quality, and speed by BetaPass developers with verified usage.",
		url: `${BASE_URL}/data/${year}`,
		creator: {
			"@type": "Organization",
			name: "betarouter",
				...
				<section className="relative overflow-hidden border-b">
					...
					<div className="container relative mx-auto max-w-3xl px-4 pt-16 pb-12 text-center motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-bottom-3 motion-safe:fill-mode-both sm:pt-20">
						...
						<p className="mx-auto mt-5 max-w-2xl text-lg leading-relaxed text-pretty text-muted-foreground">
							The {year} BetaPass Model Census: value, quality, and speed —
							rated only by developers who shipped with these models. Every
							rating is backed by at least 50 real requests through betarouter
							in the past 30 days. No benchmarks, no vibes.
						</p>
						...
						{models.length > 0 ? (
							...
							<div className="rounded-lg border border-dashed border-stone-400/70 bg-stone-50/70 p-10 text-center dark:border-stone-600/70 dark:bg-stone-900/30">
								...
								<p className="mx-auto mt-4 max-w-md text-sm text-muted-foreground">
									{results
										? `Models appear here once ${results.minResponses} developers have filed verified entries on them. BetaPass members: your census entry gets the registry moving — and earns you a free Reset Pass.`
							...
							{[
								{
									...
									body: "You can only rate a model your BetaPass workspace has hit with 50+ requests in the past 30 days. Nobody rates a model they read a thread about.",
								},
								{
									title: "Members only",
									body: "Every respondent has an active, paid BetaPass plan. These are verdicts from people spending their own credits.",
								},
								{
