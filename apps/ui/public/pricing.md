# betarouter — Pricing

Last updated: 2026-07-10

> One OpenAI-compatible API for 200+ models across 40+ providers. Per-token prices match each provider's published rates; betarouter charges a flat 5% platform fee when you buy credits. Bringing your own provider keys is free.

## Managed cloud — pay-as-you-go credits

- Price: $0 subscription. Prepaid credits, $10 minimum / $5,000 maximum per top-up.
- Platform fee: flat 5% added at credit purchase (plus 1.5% for international cards).
- Token pricing: provider list rates per token — no markup on tokens. Live per-model prices: https://betarouter.com/models
- Includes: 200+ models, 40+ providers, automatic failover, response caching, usage analytics, and cost tracking.

## Bring Your Own Keys (BYOK)

- Price: free. Your provider bills you directly at its own rates.
- Platform fee: none — the 5% fee applies only to credit purchases.

## Enterprise

- Price: custom — volume discounts, custom routing, unlimited data retention, 99.9% uptime SLA.
- Details: https://betarouter.com/enterprise — contact contact@betarouter.com

## Related products

- BetaPass — flat-price dev plans for AI coding tools: Lite $29/month ($87 model usage included), Pro $79/month ($237 included), Max $179/month ($537 included). https://betapass.betarouter.com
- Chat — consumer AI chat plans, billed monthly: Starter $9, Plus $19, Pro $49. https://chat.betarouter.com

## Notes

- The API is OpenAI-compatible: point your existing SDK at the gateway base URL.
- Free models are available on the free tier without buying credits.
- Human-readable pricing page: https://betarouter.com/pricing
