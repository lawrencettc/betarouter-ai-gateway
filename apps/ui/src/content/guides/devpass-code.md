...
id: devpass-code
slug: devpass-code
title: BetaPass Code Integration
description: Set up BetaPass Code, the open-source terminal coding agent built for betarouter. Install from npm, log in once from your browser, and code with every model on the gateway.
date: 2026-07-03
---

BetaPass Code is an open-source terminal coding agent that talks only to betarouter. It's a fork of opencode (MIT) trimmed down to a single upstream: one browser login, roughly 190 text models, and no per-provider API keys to juggle. Every request is tagged with `x-source: devpass-code`, so usage is attributed correctly in your dashboard and on your BetaPass plan.

## Install

Install BetaPass Code globally from npm:
...
BetaPass Code supports one-click browser login, just like Claude Code:
...
Every text model on the gateway shows up in the model picker, and switching is a keystroke — no config change. All requests and their costs appear in your [dashboard](https://betarouter.com/dashboard).
...
- **BetaPass billing** — A [BetaPass subscription](/code) is applied automatically, with usage attributed per agent
...
Read the [announcement post](/blog/devpass-code) for the full story, or the [docs guide](https://docs.betarouter.com/guides/devpass-code) for reference details.
