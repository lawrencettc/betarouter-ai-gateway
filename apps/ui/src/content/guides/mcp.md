...
description: Use betarouter's built-in MCP server to give Claude Code, Codex, Cursor, or any MCP client access to 200+ models — chat, image generation, and model discovery as tools.
date: 2026-07-03
---

betarouter ships a hosted [Model Context Protocol](https://modelcontextprotocol.io) (MCP) server at `https://api.betarouter.com/mcp`. Connect it to Claude Code, Codex, Cursor, or any MCP-compatible client and your AI assistant gets tools to call **any model in our catalog** — ask GPT-5 for a second opinion from inside Claude Code, generate images mid-session, or look up model pricing without leaving your editor.
...
You'll need an API key from the [betarouter dashboard](/dashboard) (**API Keys** section).
...
Any other MCP client works the same way: streamable HTTP transport, `https://api.betarouter.com/mcp`, bearer auth.
...
Every tool call is a normal betarouter request — it shows up in your [dashboard](/dashboard) with cost and token counts, hits the cache when repeated, and uses the same credits as your API traffic.
...
For the full tool parameter reference, see the [MCP docs](https://docs.betarouter.com/guides/mcp).
