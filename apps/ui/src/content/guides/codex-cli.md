...
description: Use any model with OpenAI's Codex CLI through betarouter. One config file, full cost tracking.
date: 2026-03-19
---

Codex CLI is OpenAI's open-source terminal coding agent. By default it connects to OpenAI's API, but with betarouter you can route it through a single gateway—use GPT-5.3 Codex, Gemini, Claude, or any of 200+ models while keeping full cost visibility.
...
openai_base_url = "https://api.betarouter.com/v1"
```
...
On first launch, Codex will prompt you for authentication. Select **Provide your own API key**, then enter your betarouter API key (starts with `llmgtwy_`).

## Why This Works

betarouter's `/v1` endpoint is fully OpenAI-compatible. Codex CLI sends requests to our gateway instead of OpenAI directly, and we route them to the right provider behind the scenes. This means:
...
The `openai_base_url` field points Codex CLI to betarouter instead of OpenAI:
...
Use `auto` to let betarouter pick the best model, or set a specific one from the [models page](https://betarouter.com/models):
...
Use `auto` to let betarouter pick the best model automatically, or choose a specific one from the [models page](https://betarouter.com/models):

```bash
# let betarouter pick the best model
model = "auto"
...
1. Go to your [organization settings](https://betarouter.com/dashboard) and navigate to **Settings > Policies**
...
If you see `401 Unauthorized` or requests going to `api.openai.com` instead of betarouter:

1. Make sure you've run `codex logout` to clear any ChatGPT session
2. Verify `openai_base_url` is set in `~/.codex/config.toml`
3. When Codex prompts for authentication, select **Provide your own API key** and enter your betarouter key (starts with `llmgtwy_`)
...
Check that `openai_base_url` is set to `https://api.betarouter.com/v1` (note the `/v1` at the end).
...
5. Run `codex`, select **Provide your own API key** when prompted, and paste your betarouter key
