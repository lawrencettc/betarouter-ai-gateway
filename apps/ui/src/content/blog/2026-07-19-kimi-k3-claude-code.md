...
summary: "Kimi K3 in Claude Code takes three environment variables. This guide walks through the exact setup for Claude Code, Cursor, and Cline via betarouter — plus what each tool does and doesn't route, and what K3 costs on a flat-rate DevPass plan."
...
**betarouter** bridges that gap. It speaks both the Anthropic and OpenAI API formats, so the tools you already use can run Kimi K3 — or any of [200+ models](https://betarouter.com/models) — with a base-URL change. Here is the exact setup for each tool.

## Kimi K3 in Claude Code

Claude Code talks to any endpoint that speaks Anthropic's `/v1/messages` format, which betarouter does natively. Three environment variables:
...
That's the whole migration. Every request now routes through betarouter to Kimi K3, and every request shows up in your dashboard with its exact cost, token counts, and cache-hit rate.
...
2. Add your betarouter key under **OpenAI API Key**
3. Enable **Override OpenAI Base URL** and set it to `https://api.betarouter.com/v1`
...
3. **Base URL**: `https://api.betarouter.com/v1`
4. **API Key**: your betarouter key
5. **Model ID**: `kimi-k3`
...
Also worth knowing: [OpenCode ships betarouter as a built-in provider](/blog/opencode-built-in-provider), so there K3 is a login and a model pick, no URLs at all.

## What Kimi K3 costs in your coding agent

Agent loops are token-hungry, which is exactly the case [DevPass](https://devpass.betarouter.com) was built for — a flat monthly rate instead of a per-token bill:
...
Yes. Claude Code sends Anthropic-format requests to whatever `ANTHROPIC_BASE_URL` points at. betarouter accepts that format and translates to each provider behind the scenes, so `ANTHROPIC_MODEL=kimi-k3` just works — as does any other model in the catalog.
...
- **[Get DevPass](https://devpass.betarouter.com)** — flat-rate Kimi K3 in your coding agent from $29/mo
- **[Try betarouter free](https://betarouter.com/signup)** — one key for K3 and 200+ models
