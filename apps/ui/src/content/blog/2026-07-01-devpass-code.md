...
slug: "devpass-code"
date: "2026-07-01"
title: "DevPass Code: A Terminal Coding Agent for betarouter"
summary: "DevPass Code is an open-source terminal coding agent that talks only to betarouter. One browser login, every model, and no per-provider API keys to juggle. Use it pay-as-you-go or on a DevPass coding subscription."
categories: ["Announcements", "Product"]
image:
  src: "/blog/devpass-code-splash.png"
  alt: "The DevPass Code terminal splash screen, showing the model set to Claude Opus 4.8 through betarouter"
  width: 1320
  height: 760
---
...
Under the hood it starts a short-lived loopback server on `localhost`, opens `https://betarouter.com/connect/cli`, and the freshly minted API key is handed straight back to your machine — it never leaves it. Prefer to paste a key you already have? Choose **Paste an API key** instead. Credentials live in `~/.local/share/devpass-code/auth.json`, and the connect page only ever redirects to a local address.

## Two providers, two ways to pay

DevPass Code ships with two providers and nothing else. Both route to `https://api.betarouter.com/v1`; the gateway decides how each request is billed from the account behind your key.

| Provider               | Use it when                                                                                                                 |
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| **betarouter**         | You pay as you go with credits or your own betarouter API key.                                                              |
| **betarouter DevPass** | You're on a [DevPass](https://betarouter.com) coding subscription — billing is handled automatically, no per-request setup. |
...
DevPass Code is a fork of [opencode](https://github.com/anomalyco/opencode) (MIT), rebranded and stripped down to a single upstream. We kept the parts that make opencode good — the fast TUI, agents, tools, MCP support, session management — and removed the provider sprawl. The result is a coding agent that does one thing: route your work through betarouter, with an interface that matches the gateway's design.
...
The tool is open source (MIT) and free to run. You pay only for the models you use through betarouter — either pay-as-you-go credits or a DevPass subscription. There's no separate charge for the agent itself.
...
No. Pick the **betarouter** provider and you're on pay-as-you-go with your own key or credits. The **betarouter DevPass** provider is there for subscribers who want flat-rate coding usage.

### Which models can I use?

Any text model betarouter supports — about 190 today across Anthropic, OpenAI, Google, xAI, DeepSeek, and more. DevPass Code fetches the live catalog, so new models appear without an update.

### How is this different from using opencode directly?

opencode supports dozens of providers and needs per-provider setup. DevPass Code is the same core with everything pointed at betarouter: one browser login, one billing relationship, and a UI themed to match the gateway.

## Get started

- **[Try betarouter free](https://betarouter.com/signup)** and grab an API key
- Run `npm i -g devpass-code` — or browse the source on [GitHub](https://github.com/theopenco/devpass-code)
- Read the [coding agents guide](https://docs.betarouter.com/guides/devpass-code) to connect it to your workflow
