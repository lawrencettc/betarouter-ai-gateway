...
date: 2026-05-15
title: "Embeddings on betarouter: One API for Vectors and Chat"
...
image:
  ...
  alt: "Embeddings on betarouter — turn meaning into vectors"
  width: 3600
  height: 1890
---
...
Starting today, that split is gone. betarouter now exposes an OpenAI-compatible **`/v1/embeddings`** endpoint. The same base URL and the same API key that handle your chat completions now handle your vectors.

## What changed

If you're already using the OpenAI SDK against betarouter, embeddings work with zero code changes:
...
If you have a betarouter dashboard open right now, embeddings traffic will appear in it without any setup.

## Getting started

1. **Pick a model.** Browse the [embedding-capable models](https://betarouter.com/models?filters=1&embedding=true) — `text-embedding-3-small` is a strong default for most use cases.
2. **Point your OpenAI client at betarouter** (if it isn't already): `baseURL: "https://api.betarouter.com/v1"`.
3. **Call `embeddings.create()`.** That's the whole integration.

Full reference and additional examples in the [embeddings docs](https://docs.betarouter.com/features/embeddings).
