...
export function buildModelFaqs(
	...
	faqs.push({
		...
		answer: `${
			...
			`${displayName} is an AI model available through betarouter.`
		} You can access it through betarouter's OpenAI-compatible API with automatic provider routing, fallback, and cost analytics.`,
	});
	...
	if (modelDef.free) {
		faqs.push({
			...
			answer: `${displayName} is currently free to use through betarouter.`,
		});
		...
		faqs.push({
			question: `How much does ${displayName} cost?`,
			answer: `Pricing for ${displayName} on betarouter starts at $${minInput.toFixed(2)} per million input tokens and $${minOutput.toFixed(2)} per million output tokens, depending on the provider. The pricing table above always reflects the current per-provider rates.`,
		});
	} else {
		faqs.push({
			question: `How much does ${displayName} cost?`,
	...
	if (providerNames.length > 0) {
		faqs.push({
			...
			answer: `${displayName} is served by ${providerNames.join(", ")} through betarouter. Requests are automatically routed to the best available provider, with fallback when a provider has issues.`,
		});
	}
	...
	faqs.push({
		...
		answer:
			hasTools && hasStructured
				? `Yes. ${displayName} supports both tool (function) calling and structured JSON outputs through betarouter.`
				: hasTools
					...
					: hasStructured
						...
						: `No. ${displayName} does not currently support tool calling or structured JSON outputs through betarouter.`,
	});
