...
import { isMappingDeactivated } from "@betarouter/shared/components";
...
export default async function ModelProviderPage({ params }: PageProps) {
	...
	const breadcrumbSchema = {
		...
		itemListElement: [
			...
			},
			{
				...
				item: `https://betarouter.com/models/${encodeURIComponent(decodedName)}/${encodeURIComponent(decodedProvider)}`,
			},
		],
	};
	...
	const productSchema = {
		...
		description:
			...
			`Access ${modelDef.name ?? modelDef.id} via ${providerInfo?.name ?? decodedProvider} through betarouter's unified API.`,
		image: `https://betarouter.com/models/${encodeURIComponent(decodedName)}/${encodeURIComponent(decodedProvider)}/opengraph-image`,
		brand: {
			"@type": "Brand",
			name: providerInfo?.name ?? decodedProvider,
		},
		...
		offers: {
			...
			url: `https://betarouter.com/models/${encodeURIComponent(decodedName)}/${encodeURIComponent(decodedProvider)}`,
			seller: {
				"@type": "Organization",
				name: "betarouter",
			},
...
export async function generateMetadata({
	...
	const description = `Pricing, latency, and capabilities for ${model.name ?? model.id} via ${providerName} on betarouter.`;
