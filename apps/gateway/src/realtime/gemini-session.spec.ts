...
function createSession(preflightOverrides: Record<string, unknown> = {}) {
	...
	const gatewayErrors = () =>
		client.sent
			...
			.filter((message) => message.betarouterError !== undefined)
			.map(
				(message) =>
					(message.betarouterError as { code: string; message: string }).code,
			);
	return {
		client,
		upstream,
...
describe("GeminiRealtimeProxySession upstream passthrough", () => {
	it("forwards a real upstream error frame unchanged", async () => {
		...
		expect(forwarded.betarouterError).toBeUndefined();
	...
	it("namespaces gateway-generated errors away from upstream errors", async () => {
		...
		expect(emitted.betarouterError).toMatchObject({
