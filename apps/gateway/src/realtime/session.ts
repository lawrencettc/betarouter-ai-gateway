...
const PROMPT_NOT_SUPPORTED = [
	...
	"Stored prompt references are not supported for realtime sessions on betarouter. Inline the instructions and tools instead.",
] as const;
				...
				if (containsImageContent(message)) {
					this.sendToClientRaw(
						buildErrorEvent(
							...
							"Image input is not yet supported for realtime sessions on betarouter.",
						),
					);
					return;
				}
	...
	private handleSessionUpdate(message: Record<string, unknown>): void {
		...
		if (requestedTranscription !== undefined) {
			if (requestedTranscription === null) {
				...
				const resolved = findRealtimeTranscriptionMapping(
					requestedModel,
					this.preflight.match.mapping.providerId,
				);
				// Bill against the catalog-admitted mapping resolved at connect
				// time, never the static packages/models entry.
				const match = resolved
					? (this.preflight.allowedTranscriptionMatches.get(resolved.modelId) ??
						resolved)
					: null;
				if (!match) {
					this.sendToClientRaw(
		...
		if (containsUnsupportedTools(session.tools)) {
			this.sendToClientRaw(
				buildErrorEvent(
					...
					"Only function tools are supported for realtime sessions on betarouter. Hosted tools (MCP, web search, etc.) are not yet available.",
				),
			);
			return;
		}
	...
	> {
		...
		if (!freshKey || freshKey.status !== "active") {
			return {
				...
				message: "The betarouter API key for this session is no longer active.",
				severity: "close",
			};
		}
		try {
