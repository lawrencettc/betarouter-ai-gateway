...
function buildGeminiError(code: string, message: string): string {
	return JSON.stringify({ betarouterError: { code, message } });
}
		...
		if (rawSetup.cachedContent !== undefined) {
			this.sendToClientRaw(
				buildGeminiError(
					...
					"Stored context caches are not supported for realtime sessions on betarouter. Inline the context instead.",
				),
			);
			return;
		}
		...
		if (
			...
			this.sendToClientRaw(
				buildGeminiError(
					...
					'Realtime sessions on betarouter are speech-to-speech: setup.generationConfig.responseModalities must be ["AUDIO"].',
				),
			);
			return;
		}
		...
		const reject = () => {
			this.sendToClientRaw(
				buildGeminiError(
					...
					"Only inline function declarations are supported for realtime sessions on betarouter. Hosted tools (Google Search, code execution, URL context, etc.) are not yet available.",
				),
			);
			return false;
		};
		...
		if (rawInput.video !== undefined) {
			this.sendToClientRaw(
				buildGeminiError(
					...
					"Video input is not supported for realtime sessions on betarouter.",
				),
			);
			return;
		}
