 ...
 * "betarouter-insecure-api-key." form is the provider-neutral alias, used by
	...
	server.on("upgrade", (req, socket, head) => {
		...
		void (async () => {
			...
			if (secretTranscriptionModel) {
				const resolvedTranscription = findRealtimeTranscriptionMapping(
					secretTranscriptionModel,
					preflight.match.mapping.providerId,
				);
				// Bill against the catalog-admitted mapping resolved by preflight,
				// never the static packages/models entry.
				allowedTranscription = resolvedTranscription
					? (preflight.allowedTranscriptionMatches.get(
							resolvedTranscription.modelId,
