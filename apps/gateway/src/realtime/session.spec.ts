...
import { listRealtimeTranscriptionMappings } from "./catalog.js";
...
/**
 * Stand-in for the catalog-admitted transcription matches preflight resolves
 * (see runRealtimePreflight): the map is what session/server bill against, so
 * the fixture must keep it in sync with the allowed ids.
 */
function buildTranscriptionMatches(
	allowedModelIds: readonly string[],
): Map<string, RealtimeMappingMatch> {
	return new Map(
		listRealtimeTranscriptionMappings("openai")
			.filter((match) => allowedModelIds.includes(match.modelId))
			.map((match) => [match.modelId, match]),
	);
}
...
function buildPreflight(): RealtimePreflightResult {
	return {
		...
		allowedTranscriptionMatches: buildTranscriptionMatches([
			"gpt-4o-mini-transcribe",
			"gpt-4o-transcribe",
		]),
...
function createSession(preflightOverrides: Record<string, unknown> = {}) {
	...
	// Overriding the allowed ids must also narrow the admitted-match map, or the
	// session would bill against a mapping the fixture says is not allowed.
	const derivedMatches = Array.isArray(
		preflightOverrides.allowedTranscriptionModelIds,
	)
		? {
				allowedTranscriptionMatches: buildTranscriptionMatches(
					preflightOverrides.allowedTranscriptionModelIds as string[],
				),
			}
		: {};
