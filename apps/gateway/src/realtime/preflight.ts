...
import { getOrganizationEnvVariant } from "@betarouter/models";

import {
	admitRealtimeMapping,
	findRealtimeMapping,
...
export interface RealtimePreflightResult {
	...
	/**
	 * The catalog-admitted transcription matches behind
	 * `allowedTranscriptionModelIds`, keyed by canonical model id. Billing must
	 * price against these (catalog customer prices) and never re-resolve the
	 * static packages/models mapping.
	 */
	allowedTranscriptionMatches: Map<string, RealtimeMappingMatch>;
	/**
...
/**
 * Validate the betarouter credential, project, organization, model and
...
async function runRealtimePreflightInner(
	...
	const staticMatch = findRealtimeMapping(input.requestedModel);
	if (!staticMatch) {
		throw new RealtimeConnectError(
			400,
			"model_not_found",
	...
	const match = await admitRealtimeMapping(staticMatch, "realtime");
	if (!match) {
		throw new RealtimeConnectError(
			503,
			"model_unavailable",
			`No eligible realtime provider mapping is available for ${input.requestedModel}`,
		);
	}
	...
	if (apiKey.status !== "active") {
		throw new RealtimeConnectError(
			401,
			"inactive_api_key",
			"Unauthorized: This betarouter API token is not active (it may be disabled or deleted).",
		);
	}
	...
	const allowedTranscriptionMatches = new Map<string, RealtimeMappingMatch>();
	for (const candidate of listRealtimeTranscriptionMappings(providerId)) {
		...
		// ASR is billed work on its own mapping, so it clears catalog admission
		// too. A catalogue rejection only removes this candidate (the session's
		// own model was already admitted above), so swallow the HTTPException
		// rather than failing the whole connection.
		let admitted: RealtimeMappingMatch | null = null;
		try {
			admitted = await admitRealtimeMapping(candidate, "realtimeTranscription");
		} catch (error) {
			if (!(error instanceof HTTPException)) {
				throw error;
			}
		...
		if (admitted) {
			allowedTranscriptionMatches.set(admitted.modelId, admitted);
		}
	}
	const allowedTranscriptionModelIds = [...allowedTranscriptionMatches.keys()];
	...
	if (project.mode === "api-keys") {
		...
		const envResult = await getProviderEnv(providerId, {
			...
			requiredCredentialId: match.mapping.platformCredentialId,
			requiredCredentialProfile: match.mapping.platformCredentialProfile,
			variant: getOrganizationEnvVariant(organization),
		});
