...
rerank.openapi(createRerank, async (c): Promise<any> => {
	...
	// Platform-catalog admission: an operator-deactivated mapping must never be
	// servable, and the mapping we go on to price with must be the
	// catalog-adjusted one (customer prices, capability restrictions, platform
	// credential binding) rather than the static packages/models entry.
	...
	const [rerankMapping] = filterProviderMappingsByCatalog(
		modelDef.providers.filter(
			(provider): provider is ProviderModelMapping =>
				Boolean((provider as ProviderModelMapping).rerank) &&
	...
	if (!rerankMapping) {
		throw new HTTPException(503, {
			message: "No eligible rerank provider mapping is available",
		});
	}
	const catalogMapping = findCatalogMappingForProvider(
		catalogDecision,
		rerankMapping.providerId,
		rerankMapping.region ?? null,
	);
	...
	async function resolveAttempt(): Promise<ResolveResult> {
		...
		if (project.mode === "api-keys") {
			...
			const envResult = await getProviderEnv(providerId, {
				...
				requiredCredentialId: rerankMapping.platformCredentialId,
				requiredCredentialProfile: rerankMapping.platformCredentialProfile,
				variant: envVariant,
			});
