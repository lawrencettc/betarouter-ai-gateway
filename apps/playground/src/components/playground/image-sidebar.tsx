...
type ImageHistoryRow =
	...
	| { type: "item"; key: string; item: GalleryItem }
...
function getImageHistoryRowHeight(
	index: number,
	{ rows }: ImageHistoryRowProps,
): number {
...
function ImageHistoryRowComponent({
	...
	onStartEdit,
	onEditFocused,
}: RowComponentProps<ImageHistoryRowProps>) {
	const row = rows[index];
	...
	return (
		<div {...ariaAttributes} style={style}>
			<div className="relative h-full px-2 pb-1">
				<div className="group/image-row relative h-full">
					{isEditing ? (
							...
							<DropdownMenu>
								<DropdownMenuTrigger asChild>
									<SidebarMenuAction
										...
										className="pointer-events-none static hidden h-7 w-7 cursor-pointer opacity-0 group-hover/image-row:flex group-hover/image-row:pointer-events-auto group-hover/image-row:opacity-100 data-[state=open]:flex data-[state=open]:pointer-events-auto data-[state=open]:opacity-100"
									>
...
function groupItemsByDate(items: GalleryItem[]) {
	...
	const groups = {
		today: [] as GalleryItem[],
		yesterday: [] as GalleryItem[],
		lastWeek: [] as GalleryItem[],
		older: [] as GalleryItem[],
	};
...
export function ImageSidebar({
	...
	const historyRows = useMemo<ImageHistoryRow[]>(() => {
		const groups: Array<{ title: string; items: GalleryItem[] }> = [
				...
				>
					{isHistoryLoading && galleryItems.length === 0 ? (
						...
						<List
							...
							style={{ width: "100%" }}
							rowComponent={ImageHistoryRowComponent}
							rowCount={historyRows.length}
							rowHeight={getImageHistoryRowHeight}
							rowProps={rowProps}
							overscanCount={8}
						/>
					)}
