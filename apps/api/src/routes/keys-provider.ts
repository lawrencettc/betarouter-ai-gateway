...
import type { ProviderKeyComplianceAttestation } from "@betarouter/db";
...
export async function assertCustomProviderNameAvailable(
	...
	// The gateway resolves `<name>/<model>` against the platform catalogue first
	// (see parseModelInput), so a custom provider that reuses a catalogue
	// provider id — including the reserved "custom" namespace itself — would be
	// permanently unroutable. Reject the collision at creation time instead of
	// silently accepting a dead provider.
	if (providers.some((item) => item.id === name)) {
		throw new HTTPException(400, {
			message: `'${name}' is a reserved provider name and cannot be used for a custom provider`,
		});
	}
