...
// The yearly BetaPass model survey only asks about models a member genuinely
// uses: their BetaPass org must have made at least this many requests on a
...
const getEligibility = createRoute({
	...
	responses: {
		200: {
			...
			description:
				"Whether the user can take this quarter's BetaPass model survey wave, and for which models.",
		},
	},
});
...
modelSurvey.openapi(submitSurvey, async (c) => {
	...
	if (!org || org.devPlan === "none") {
		throw new HTTPException(403, {
			message:
				"The model survey is only open to BetaPass members with an active plan.",
		});
	}
		...
		({ row, rewardTier } = await db.transaction(async (tx) => {
			...
			if (grantTier) {
				...
				await tx.insert(tables.transaction).values({
					...
					currency: "USD",
					status: "completed",
					description: `BetaPass Reset Pass (${grantTier.toUpperCase()}) — ${year} Q${quarter} model survey reward`,
				});
			}
