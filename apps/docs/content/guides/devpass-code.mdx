---
title: DevPass Code
description: DevPass Code is a terminal coding agent that talks only to betarouter. One-click browser login, ~190 models, and DevPass subscription billing handled automatically.
icon: DevPassCode
---
...
Choose either **betarouter** (pay-as-you-go with your own API key) or **betarouter DevPass** (the DevPass coding subscription). See [Providers](#providers) below for the difference.
...
Select **"Log in with browser."** DevPass Code opens [https://betarouter.com/connect/cli](https://betarouter.com/connect/cli) and starts a local loopback server.
...
DevPass Code ships exactly two providers. Both hit `https://api.betarouter.com/v1` (OpenAI-compatible) and expose the same ~190 text models — the only difference is how usage is billed.

| Provider               | Billing                                                                                                 |
| ---------------------- | ------------------------------------------------------------------------------------------------------- |
| **betarouter**         | Pay-as-you-go with your own API key                                                                     |
| **betarouter DevPass** | The DevPass coding subscription — billing is handled automatically by the gateway based on your account |
...
Once authenticated, start DevPass Code in any project directory and describe what you want to build. All requests route through betarouter with `x-source: devpass-code`, so every prompt and its cost appear in your [dashboard](https://betarouter.com/dashboard).
...
## Why Use betarouter with DevPass Code
