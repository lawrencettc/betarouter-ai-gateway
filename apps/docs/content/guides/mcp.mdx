...
description: Use betarouter as an MCP server for Claude Code, Cursor, and other MCP-compatible clients
icon: Mcp
---
...
betarouter provides a Model Context Protocol (MCP) server that enables AI assistants like Claude Code to access multiple LLM providers through a unified interface. This allows you to use any model from OpenAI, Anthropic, Google, and more directly from your AI coding assistant.
...
The Model Context Protocol (MCP) is an open standard that allows AI assistants to connect with external tools and data sources. betarouter's MCP server exposes tools for:
...
<Tabs items={["Claude Code", "Codex", "Cursor", "Other MCP Clients"]}>
  ...
  <Tab value="Codex">
    ...
    2. Navigate to **API Keys** section
    3. Create a new API key and copy it
    4. Set it as an environment variable: `export BETA_GATEWAY_API_KEY="your-api-key-here"`
    ...
    Run `/mcp` in the Codex TUI to confirm the `betarouter` server is connected. Try:
  ...
  <Tab value="Cursor">
    ...
    Open a chat in **Agent Mode**, click the **Select Tools** icon, and verify the betarouter tools appear. Try:
  ...
  <Tab value="Other MCP Clients">
    betarouter's MCP server supports the standard HTTP Streamable transport. Configure your client with:

    - **Endpoint:** `https://api.betarouter.com/mcp`
    ...
    **Direct HTTP Example:**
    ...
    ```bash
    curl -X POST https://api.betarouter.com/mcp \
    ...
    **Server-Sent Events (SSE):**
    ...
    ```bash
    curl -N https://api.betarouter.com/mcp \
...
Your API key is the same one you use for the REST API and works across all betarouter services.

## OAuth Support

For applications that prefer OAuth authentication, betarouter's MCP server implements OAuth 2.0:
...
2. Check the endpoint URL is correct: `https://api.betarouter.com/mcp`
...
3. Verify the MCP server is responding: `GET https://api.betarouter.com/mcp`
...
- **Cost Tracking** - Monitor usage and costs in the betarouter dashboard
