...
**Agent Skills** are structured guidelines for AI coding agents, optimized for use with betarouter and the AI SDK. They provide best practices and reusable instructions that help AI agents generate higher-quality code.
...
The Changelog Writer skill drafts a polished betarouter changelog entry and its OpenGraph image in one pass:
...
Place the folder in `~/.claude/skills/` instead to make the skill available across every project.
