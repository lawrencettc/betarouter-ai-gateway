...
Hermes Agent is fully compatible with [BetaPass coding plans](/features/coding-agents). The gateway automatically detects Hermes via multiple signals:
...
Make sure your `api_key` starts with `llmgtwy_` and is valid. Check your [dashboard](https://betarouter.com/dashboard) to confirm the key is active.
