...
Voice Calls turns the Playground into a phone line to a model. Pick a realtime model and a voice, start the call, and talk — the model listens, answers out loud, and the transcript builds as you go. Every call runs over the same [Realtime API](/features/realtime) your own apps can use.

<Callout type="warn">
	Voice Calls depends on the [Realtime API](/features/realtime), which is off by
	default and has to be enabled by the operator of your betarouter deployment.
	Until it is, the Voice studio tile is hidden and `/realtime` is not reachable.
</Callout>
...
<Callout type="info">
	...
	organization switcher on this page only lists those — BetaPass and chat-plan
...
The same endpoint backs the [Realtime API](/features/realtime): connect a WebSocket to `wss://api.betarouter.com/v1/realtime`, authenticate with an [API key](/learn/api-keys), and use the standard OpenAI realtime event protocol. For browser clients, mint an ephemeral client secret from your backend instead of shipping a key:
