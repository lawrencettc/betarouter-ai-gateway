...
Member-level rules are a **ceiling**: they apply to every API key the member creates, and the member's own key rules can only narrow access further, never expand it. For example, if you restrict a member to a single approved [provider](https://betarouter.com/providers), they can still create a key limited to one of that provider's models, but no key of theirs can reach any other provider.
