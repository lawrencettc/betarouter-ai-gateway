...
<Callout type="warn">
	Realtime is currently in a staged hosted rollout. Until it is enabled for the
	service, `POST /v1/realtime/client_secrets` returns 404 and the WebSocket
	upgrade is refused. Realtime requires a regular developer API key (not an
	end-user session token) on a pay-as-you-go organization with credits or a BYOK
	provider key without a custom base URL.
</Callout>
