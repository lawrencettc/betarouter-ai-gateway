...
betarouter supports low-latency, speech-to-speech conversations through the
...
`wss://api.betarouter.com/v1/realtime`, authenticate with your betarouter API
key, and keep your existing event handling.

<Callout type="warn">
	Realtime is **off by default** and must be turned on by the operator of your
	betarouter deployment. Until it is, `POST /v1/realtime/client_secrets` returns
	404 and the WebSocket upgrade is refused. Self-hosting? See the
	enable-realtime procedure in `infra/DEPLOYMENT_BETAROUTER.md`. Realtime also
	requires a regular developer API key (not an end-user session token) on a
	pay-as-you-go organization with credits or a BYOK provider key without a
	custom base URL.
</Callout>

<Callout type="info">
	Want to talk to a model right now? The [Realtime
	page](https://chat.betarouter.com/realtime) in the Playground runs a full
	voice call in the browser using this API.
</Callout>

## Available Models

Browse the available realtime models, with up-to-date pricing, on the
[models page](https://betarouter.com/models). Realtime sessions are billed per
...
Model names work like everywhere else on betarouter: use the plain model id
...
```
wss://api.betarouter.com/v1/realtime?model=gpt-realtime
```

Authenticate with your betarouter API key in the `Authorization` header (an
`x-api-key` header also works):

```javascript
import WebSocket from "ws";

const url = "wss://api.betarouter.com/v1/realtime?model=gpt-realtime";
...
```bash
curl -X POST "https://api.betarouter.com/v1/realtime/client_secrets" \
...
```javascript
const ws = new WebSocket("wss://api.betarouter.com/v1/realtime", [
	"realtime",
...
[models page](https://betarouter.com/models) for available realtime
...
billed. Usage appears in your [activity feed](https://betarouter.com/dashboard)
...
- Realtime requires a regular developer API key on a pay-as-you-go
  organization. End-user session tokens, platform keys, and BetaPass/chat plan
