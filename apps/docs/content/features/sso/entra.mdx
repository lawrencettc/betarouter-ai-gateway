...
This guide walks an organization admin through connecting **Microsoft Entra ID** (formerly Azure Active Directory) to betarouter for SAML single sign-on and SCIM provisioning.
...
An **Enterprise application** is a registered instance of an app that your tenant trusts for sign-in. You'll create one to represent betarouter; its **Single sign-on (SAML)** page is where you exchange URLs and download the certificate.
...
Click **+ New application** → **+ Create your own application**. Name it (e.g. `betarouter`), choose **"Integrate any other application you don't find in the gallery (Non-gallery)"**, then **Create**. Wait a few seconds for it to provision.

<Callout type="warn">
	Don't search the Entra **gallery** for "betarouter" — there is no gallery
...
- **Identifier (Entity ID):** `https://platform-api.betarouter.com/auth/sso/saml2/sp/metadata?providerId=acme-entra`
- **Reply URL (ACS):** `https://platform-api.betarouter.com/auth/sso/saml2/sp/acs/acme-entra`
...
**Section 2 – Attributes & Claims**: the Entra defaults are fine. In **Microsoft Entra ID** mode, betarouter reads email/name from the standard claim URIs Entra sends by default (`.../emailaddress`, `.../givenname`, `.../surname`, `.../name`), so you don't need to force the Name ID to be the email — just make sure users have a real `mail`/UPN.
...
<Callout type="warn">
	...
	email**. betarouter signs a user in from the `emailaddress` claim, which Entra
...
- **Authentication method:** choose the token option — labelled **"Long-lived bearer token"** (or **"Secret token"**). **Do not** pick "OAuth 2.0 code grant flow": betarouter's SCIM endpoint authenticates a plain static bearer token (`Authorization: Bearer <token>`), not an OAuth code exchange. The presence of a **Tenant URL + Secret token** pair on the form confirms you're on the right method.
- **Tenant URL:** the SCIM base URL, e.g. `https://platform-api.betarouter.com/scim/v2`
...
Assigned users are created in betarouter and added to the organization (default role **developer**). Deactivating or unassigning a user in Entra removes their membership.
...
Log out → login page → **Sign in with SSO** → enter a work email (`someone@acme.com`). betarouter resolves the connection by email domain, redirects to Entra, and on return the (SCIM-provisioned) user is signed in and lands in the enterprise organization.
...
If the Entra sign-in itself succeeds but the user lands back on the betarouter login page (with a 401 in the browser console), the SAML response was received but betarouter refused to attach it to the provisioned user — almost always an `account_not_linked` domain mismatch:
