...
This guide walks an organization admin through connecting **Okta** to betarouter for SAML single sign-on and SCIM provisioning.
...
You'll create a custom SAML 2.0 app integration in the Okta Admin Console to represent betarouter. There is no OIN (Okta Integration Network) catalog listing — the custom app is the supported path, and the steps below cover everything.

<Steps>
<Step>
### Choose a connection slug in betarouter

In betarouter (**Organization → SSO → Add a connection**), set **Identity provider** to **Okta**, then pick a **connection slug** (lowercase letters, numbers, hyphens; `^[a-z0-9-]+$`, up to 64 chars). We recommend the format `<your-org-slug>-<provider>`, e.g. `acme-okta` — the form pre-fills this from your organization name and the selected provider. It must be unique across all betarouter organizations, and each organization has exactly one SSO connection. Keep it stable: your SP URLs are built from it.

As soon as the slug is set, the form shows your two SP URLs (with the hosted default `https://platform-api.betarouter.com`) — copy them from there:

- **Audience URI (SP Entity ID):** `https://platform-api.betarouter.com/auth/sso/saml2/sp/metadata?providerId=acme-okta`
- **Single sign-on URL (ACS):** `https://platform-api.betarouter.com/auth/sso/saml2/sp/acs/acme-okta`
...
Click **Create App Integration**, choose **SAML 2.0** as the sign-in method, and click **Next**. On **General Settings**, set the **App name** (e.g. `betarouter`) and click **Next**.

<Callout type="warn">
	Pick **SAML 2.0**, not **OIDC**: betarouter's per-organization SSO speaks
...
The **Show Advanced Settings** defaults are correct (Response **Signed**, Assertion Signature **Signed**, RSA-SHA256) — betarouter requires signed assertions, which Okta does by default.
...
<Callout type="warn">
	**Name ID format = EmailAddress** and **Application username = Email** are
	what make sign-in work: in **Okta** mode, betarouter reads the user's email
...
<Callout type="warn">
	Use this exact app-specific **Sign on URL** — betarouter derives the SAML
...
<Step>
### Create the connection in betarouter
...
If you also set up SCIM below, assignment doubles as the provisioning trigger: assigned users are pushed to betarouter.
...
In betarouter, on **Organization → SSO → Directory sync (SCIM)**, click **Generate SCIM token** and copy it (shown once). The SCIM base URL is shown in the same card — it's `https://platform-api.betarouter.com/scim/v2` on the hosted default.
...
- **SCIM connector base URL:** the SCIM base URL, e.g. `https://platform-api.betarouter.com/scim/v2`
- **Unique identifier field for users:** `userName`
- **Supported provisioning actions:** check **Push New Users**, **Push Profile Updates**, and **Push Groups** (leave the two _Import_ options unchecked — betarouter doesn't support importing users into Okta)
- **Authentication Mode:** **HTTP Header** — Okta then shows an **Authorization: Bearer** field; paste the SCIM token there. **Do not** pick Basic Auth or OAuth 2: betarouter's SCIM endpoint authenticates a plain static bearer token.
...
From now on, assigning a user (or an assigned group's member) pushes them to betarouter; unassigning or deactivating them removes their membership.
...
Provisioned users are created in betarouter and added to the organization (default role **developer**). Deactivating or unassigning a user in Okta removes their membership.

## Group → role mapping (optional)

On the app's **Push Groups** tab, push the Okta groups whose membership should matter. Then on the betarouter **SSO** page, add mappings such as `Acme Admins → admin`. When Okta pushes group membership, each member gets the **highest** mapped role among their groups (default `developer`). Owners are never auto-demoted, so you can't accidentally lock yourself out of the owner role.
...
- **Okta sign-in succeeds but the user lands back on the betarouter login page** — the SAML response was received but refused, almost always an `account_not_linked` domain mismatch: the asserted email's domain isn't in the connection's **Email domains** list. Edit the list on the connection card to cover every domain Okta may assert.
