...
As soon as the slug is set, the form shows your two SP URLs (with the hosted default `https://internal.betarouter.com`) — copy them from there:

- **Audience URI (SP Entity ID):** `https://internal.betarouter.com/auth/sso/saml2/sp/metadata?providerId=acme-okta`
- **Single sign-on URL (ACS):** `https://internal.betarouter.com/auth/sso/saml2/sp/acs/acme-okta`
...
In betarouter, on **Organization → SSO → Directory sync (SCIM)**, click **Generate SCIM token** and copy it (shown once). The SCIM base URL is shown in the same card — it's `https://internal.betarouter.com/scim/v2` on the hosted default.
...
- **SCIM connector base URL:** the SCIM base URL, e.g. `https://internal.betarouter.com/scim/v2`
