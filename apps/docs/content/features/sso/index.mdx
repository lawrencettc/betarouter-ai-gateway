...
betarouter supports per-organization **SAML 2.0 single sign-on (SSO)** and **SCIM 2.0 directory provisioning**, so an enterprise team can let members sign in with their corporate identity provider and keep membership in sync automatically.
...
<Callout type="info">
	...
	Contact us at contact@betarouter.com to enable it.
</Callout>
...
## The URLs betarouter gives your IdP

Everything your identity provider needs is derived from the **management API base URL** and a **Connection ID** you choose. On the hosted service the base URL defaults to `https://internal.betarouter.com` (overridable via `API_URL`); self-hosted deployments use whatever `API_URL` is set to.

<Callout type="warn">
	The management API base URL is **not** `api.betarouter.com` — that host serves
...
With `API_URL = https://internal.betarouter.com` and a connection slug of `acme-entra`:

| Value                             | URL                                                                                |
| --------------------------------- | ---------------------------------------------------------------------------------- |
| Identifier (Entity ID) / Audience | `https://internal.betarouter.com/auth/sso/saml2/sp/metadata?providerId=acme-entra` |
| Reply URL (ACS)                   | `https://internal.betarouter.com/auth/sso/saml2/sp/acs/acme-entra`                 |
| SCIM Tenant URL                   | `https://internal.betarouter.com/scim/v2`                                          |
...
- **Authentication method:** the **bearer token** option — Entra labels it **"Long-lived bearer token"** / **"Secret token"**; Okta calls it an **API token**. Do **not** choose an OAuth 2.0 authorization-code flow — betarouter does not implement one, and the connection test will fail.
...
- [Okta](/features/sso/okta) — set up SAML SSO and SCIM provisioning with a custom Okta app integration.
- [Google Workspace](/features/sso/google) — no SAML app needed: members sign in with the standard **Sign in with Google** button and auto-join your organization by email domain.
...
3. betarouter resolves the domain `acme.com` → finds the org's SSO connection → redirects to the org's IdP.
