...
You can use the AI SDK to generate images with your existing generateText or streamText calls using the betarouter provider.
...
export async function POST(req: Request) {
	...
	try {
		...
		return new Response(
			JSON.stringify({ error: "betarouter Chat request failed" }),
			{
				status: 500,
			},
		);
