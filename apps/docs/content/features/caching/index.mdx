---
title: Caching
description: Overview of the two types of caching available in betarouter.
icon: Zap
---
...
betarouter supports **two distinct kinds of caching**, and they solve different problems. Pick the one that matches your workload — they can also be used together.
...
betarouter performs the caching. When a request is **byte-identical** to a previous one (same model, same messages, same parameters), the response is served from the gateway's cache without any provider call. Repeated identical calls cost **$0**.
