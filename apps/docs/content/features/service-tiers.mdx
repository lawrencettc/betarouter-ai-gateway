...
OpenAI-compatible `service_tier` parameter, and betarouter forwards it only
...
Service tiers do not define separate model prices in betarouter. They multiply
...
When a provider reports the tier that was actually served, betarouter bills
...
<Callout type="warn">
	betarouter rejects unsupported tier requests before provider routing. For
...
[model page](https://betarouter.com/models). Supported provider cards include a
