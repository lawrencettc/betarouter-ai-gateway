...
betarouter exposes a dedicated `/v1/audio/transcriptions` endpoint for
...
`POST https://api.betarouter.com/v1/audio/transcriptions`
...
```bash
curl -X POST "https://api.betarouter.com/v1/audio/transcriptions" \
  -H "Authorization: Bearer $LLM_GATEWAY_API_KEY" \
