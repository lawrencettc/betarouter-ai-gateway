...
betarouter provides real-time cost information for each API request directly in the response's `usage` object. This allows you to track costs programmatically without needing to query the dashboard.
...
| `cost_details.total_cost`                          | Total request cost in USD (betarouter extended field)                                                                                                  |
...
If you're running a self-hosted betarouter deployment, cost breakdown is always included in API responses regardless of plan. This allows you to track internal costs and allocate them across teams or projects.
