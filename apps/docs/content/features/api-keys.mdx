...
API keys are the primary method for authenticating with the betarouter. This guide covers creating API keys, managing them, and configuring IAM rules for fine-grained access control.

## Overview

betarouter provides comprehensive API key management with the following features:
...
gateway as a bearer token. betarouter also issues three other kinds of keys,
...
1. Navigate to your project in the betarouter dashboard
...
curl -X POST "https://api.betarouter.com/v1/chat/completions" \
  -H "Authorization: Bearer beta_your_api_key_here" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "gpt-4o",
...
from betarouter credits and usage from your own provider keys when applicable,
