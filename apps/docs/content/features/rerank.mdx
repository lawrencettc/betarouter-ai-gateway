...
betarouter exposes a Cohere-compatible `/v1/rerank` endpoint that scores a list
...
[models page](https://betarouter.com/models?filters=1&rerank=true).
...
`POST https://api.betarouter.com/v1/rerank`

## cURL

```bash
curl -X POST "https://api.betarouter.com/v1/rerank" \
...
const res = await fetch("https://api.betarouter.com/v1/rerank", {
	method: "POST",
	headers: {
