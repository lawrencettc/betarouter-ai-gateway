...
<Callout type="info">
	...
	plan**](https://betarouter.com/enterprise) for organization owners and admins.
</Callout>
...
The selector only offers countries that are referenced by a provider in the catalogue — browse the [providers directory](https://betarouter.com/providers) to see the current set and each provider's headquarters. Selecting no country applies no location restriction. The country filter is also fail-closed: when at least one country is selected, a provider whose headquarters is unknown is treated as non-compliant.
...
{
	"error": {
		"message": "This request was blocked by your organization's provider compliance policy. No available provider for deepseek-v3.2 meets the required certifications or provider/model restrictions. Contact your betarouter admin to adjust the policy."
	}
}
```
...
<Callout type="warn">
	betarouter does not verify attestations — they are your organization's own
...
**Known limitation:** the country selector in the compliance policy only offers countries referenced by catalogue providers (see the [providers directory](https://betarouter.com/providers)). A custom deployment attested to a country outside that set cannot be allowed while a country restriction is active.
...
Provider compliance policies are an Enterprise feature. [Contact us](https://betarouter.com/enterprise) to enable Enterprise for your organization.
