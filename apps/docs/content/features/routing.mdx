---
title: Routing
description: Learn how betarouter intelligently routes your requests to the best available models and providers.
icon: Route
---
...
betarouter provides flexible and intelligent routing options to help you get the best performance and cost efficiency from your AI applications. Whether you want to use specific models, providers, or let our system automatically optimize your requests, we've got you covered.

betarouter also includes **automatic retry and fallback** — if a provider fails, your request is seamlessly retried on the next best provider, all within the same API call.

## Model Selection

### Any Model Name

You can use any model name from our [models page](https://betarouter.com/models) or discover available models programmatically through the [/v1/models endpoint](/v1_models).
...
Choose a specific model ID to route to the **best available provider** for that model. betarouter's smart routing algorithm considers multiple factors to find the optimal provider across all configured options.

#### Smart Routing Algorithm

When you use a model ID without a provider prefix, betarouter's intelligent routing system analyzes multiple factors to select the best provider.
...
The exploration rate is configurable per project through the routing configuration (`thresholds.explorationRate`).

**Stable Provider Preference**:

To avoid unnecessary churn between providers that score similarly, betarouter remembers the best provider chosen for each model and sticks with it across requests — even if another provider edges ahead slightly on the next score calculation.
...
<Callout type="info">
	On the **Enterprise plan**, the preference lifetime, hard-switch uptime floor,
	and soft-switch score gap can be customized per project from the dashboard —
...
| Strategy           | Behavior                                                                             |
| ------------------ | ------------------------------------------------------------------------------------ |
| `auto` _(default)_ | Full weighted smart-routing score (price, uptime, throughput, latency, cache).       |
...
Sticky session routing solves this: attach a session identifier and betarouter pins all requests for that session to a single provider (and region), keeping the upstream prompt cache warm across the whole conversation.
...
<Callout type="info">
	Provider-specific routing is not available on **BetaPass coding plans** —
	provider-prefixed model ids (and custom provider routing) return a `403`
	there. BetaPass always uses the prompt-cache–aware smart routing above with
	plain model ids. Provider pinning requires the pay-as-you-go API. See
	[Provider routing on
	BetaPass](/learn/model-categories#provider-routing-on-betapass).
</Callout>

#### Regions

Some providers expose the same model in multiple regions. In that case, betarouter supports two routing modes:
...
```bash
# Let betarouter choose the best Alibaba region for DeepSeek V3.2
curl -X POST "https://api.betarouter.com/v1/chat/completions" \
  -H "Authorization: Bearer $BETA_GATEWAY_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
If your provider key stores an explicit region, that region acts like a lock and betarouter will only use that region for provider-specific requests. If no explicit region is configured on the provider key, provider-specific requests can still score all eligible regions for that provider.
...
When you specify a provider explicitly, betarouter checks the provider's recent uptime (from the time-decayed metrics window described above). If the uptime falls below 90%, the system automatically routes your request to the best available alternative provider to ensure reliability. This protects your application from providers experiencing temporary issues. The fallback threshold (default `90%`) is configurable.
...
When using model ID routing (without a provider prefix), betarouter automatically retries failed requests on alternate providers. This happens transparently within the same API call — your application receives the successful response as if nothing went wrong.
...
<Callout type="info">
	...
	to tune routing for your workloads, contact us at contact@betarouter.com.
</Callout>
...
```bash
# Let betarouter choose the optimal model
curl -X POST "https://api.betarouter.com/v1/chat/completions" \
  -H "Authorization: Bearer $BETA_GATEWAY_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
