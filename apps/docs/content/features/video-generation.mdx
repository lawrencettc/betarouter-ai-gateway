...
betarouter supports asynchronous video generation through an OpenAI-compatible `POST /v1/videos` flow.
...
You can find the current list of video-capable models on our [models page with the video filter enabled](https://betarouter.com/models?filters=1&videoGeneration=true) or programmatically through the [/v1/models endpoint](/v1_models).
...
betarouter currently supports a focused subset of the OpenAI video API.
...
| `callback_url`     | string  | no       | betarouter extension for completion webhooks                                                                               |
| `callback_secret`  | string  | no       | betarouter extension used to sign webhook deliveries                                                                       |
...
```bash
curl -X POST "https://api.betarouter.com/v1/videos" \
...
```bash
curl "https://api.betarouter.com/v1/videos/v_123" \
  -H "Authorization: Bearer $LLM_GATEWAY_API_KEY"
```
...
```bash
curl "https://api.betarouter.com/v1/videos/v_123/content" \
  -H "Authorization: Bearer $LLM_GATEWAY_API_KEY" \
  --output video.mp4
```
...
betarouter can notify your application when the job reaches a terminal state.
