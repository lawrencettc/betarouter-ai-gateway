...
betarouter supports the OpenAI-compatible `/v1/moderations` endpoint for text
...
`POST https://api.betarouter.com/v1/moderations`

Authenticate with your betarouter API key:
...
```bash
curl -X POST "https://api.betarouter.com/v1/moderations" \
  -H "Authorization: Bearer $LLM_GATEWAY_API_KEY" \
...
betarouter content filtering on `/v1/chat/completions` instead.
