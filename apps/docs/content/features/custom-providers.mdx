...
description: Learn how to integrate custom OpenAI-compatible providers with betarouter for enhanced flexibility and control.
icon: Columns3Cog
---
...
betarouter supports integrating custom OpenAI-compatible providers, allowing you to use any API that follows the OpenAI chat completions format. This feature is perfect for:
...
- betarouter will append `/v1/chat/completions` automatically
...
Custom providers inherit full betarouter functionality.
...
enforced limits** — betarouter has no catalog entry for the model, so it cannot
know its pricing, context window, or capabilities. The **custom model catalog**
lets you define that information per custom provider key, so betarouter can
...
When a request matches a catalog entry, betarouter bills it using the entry's
...
When an entry defines limits or capabilities, betarouter enforces them before
...
posture under **Custom Models → Compliance attestation**, which the gateway then
evaluates against the policy exactly like a catalogue provider's data policy.
See [Compliance → Custom providers](/features/compliance#custom-providers) for
details and caveats.
