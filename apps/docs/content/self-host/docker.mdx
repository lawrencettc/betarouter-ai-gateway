---
title: Docker
description: Run betarouter as a single Docker container — the fastest way to a working instance.
icon: Container
---

The unified Docker image bundles every service — UI, API, Gateway, PostgreSQL, and Redis — into a single container. It's the fastest way to get a working instance and is ideal for trying betarouter out or running a single low-traffic deployment.
...
betarouter supports multiple API keys per provider for load balancing — provide comma-separated values. See [Docker Compose](/self-host/docker-compose#multiple-api-keys-and-load-balancing) for details.
