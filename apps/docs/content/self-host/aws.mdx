---
title: AWS
description: Deploy betarouter on AWS with EKS, RDS for Postgres, ElastiCache, and Secrets Manager.
icon: Cloud
---
...
This guide covers a production deployment of betarouter on AWS using EKS for the application services and managed AWS services for the backing stores.
...
With the backing services in place, deploy betarouter with the [Helm chart](/self-host/kubernetes), pointing it at your RDS and ElastiCache endpoints:
...
<Callout type="info">
	**Prefer not to wire this up by hand?** The [Enterprise
	...
	deploy betarouter in one command. [Talk to
