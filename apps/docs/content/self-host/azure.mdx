---
title: Azure
description: Deploy betarouter on Azure with AKS, Azure Database for PostgreSQL, Azure Cache for Redis, and Key Vault.
icon: Cloud
---
...
<Callout type="info">
	**Deploy in one command.** Our [Enterprise
	plan](https://betarouter.com/enterprise) includes Terraform modules that
	...
	more](https://betarouter.com/enterprise).
</Callout>

This guide covers a production deployment of betarouter on Azure using AKS for the application services and managed Azure services for the backing stores.
...
With the backing services in place, deploy betarouter with the [Helm chart](/self-host/kubernetes), pointing it at your Azure Database and Azure Cache endpoints:
...
<Callout type="info">
	**Prefer not to wire this up by hand?** The [Enterprise
	plan](https://betarouter.com/enterprise) ships Terraform modules that stand up
	the entire Azure stack — AKS, Azure Database for PostgreSQL, Azure Cache for
	Redis, networking, and secrets — and deploy betarouter in one command. [Talk
	to us](https://betarouter.com/enterprise#contact).
</Callout>
