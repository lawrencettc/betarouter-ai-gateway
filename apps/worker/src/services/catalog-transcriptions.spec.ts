...
// Transcriptions-slice exit gate: an operator activates a static
// speech-to-text mapping through the admission gates — credential, a passed
// minimal-transcriptions probe (a chat probe must NOT satisfy it), a price
// policy, and enablement — and the catalog request policy then serves it
// with the audio-hour duration prices the transcription surface replays
// synchronously at request time.
const CANARY_MODEL_ID = "grok-stt-1-0";
const CANARY_PROVIDER_ID = "xai";
const ACTOR = "transcriptions-slice";
...
async function applyOperations(operations: CatalogOperationV1[]) {
	...
	const [changeSet] = await db
		...
		.values({
			createdBy: ACTOR,
			title: "Transcriptions slice activation test",
			reason: "Activate a transcription mapping through the admission gates",
			state: "draft",
...
describe("catalog transcriptions activation", () => {
	beforeEach(async () => {
		resetCatalogRevisionSnapshotCache();
		await cleanTables();
	});
	...
	it("activates a transcription mapping only through the minimal-transcriptions probe and serves its prices", async () => {
		...
		const [modelRow] = await db
			...
			.limit(1);
		expect(modelRow?.output).toEqual(["transcription"]);
		...
		// The audio-hour price rides the mirror so admission pricing works for
		// admin-edited and admin-created mappings alike.
		expect(Number(mappingRow!.inputAudioHourPrice)).toBeCloseTo(0.1);
		...
		// A passed CHAT probe on the transcription mapping must not satisfy the
		...
		// The minimal-transcriptions probe with the same fingerprint-bound
		// target is what unlocks activation and routing.
		await db.insert(platformMappingTestRun).values({
			createdBy: ACTOR,
			mappingId,
			credentialId: credential!.id,
			...
			testProfile: catalogMappingTestProfile({
				...profileTarget,
				profile: "minimal-transcriptions",
			}),
		});
		...
		// Duration billing flows through the price policy as the flat
		// USD-per-hour audioHour unit (never a per-million unit): the static
		// $0.10/hour mapping surfaces unchanged, and the transcription surface
		// multiplies it by the reported duration at request time.
		expect(Number(effective?.sourcePrices.audioHour)).toBeCloseTo(0.1);
		expect(Number(effective?.customerPrices.audioHour)).toBeCloseTo(0.1);
