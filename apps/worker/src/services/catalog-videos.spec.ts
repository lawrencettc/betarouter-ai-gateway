...
import { getVideoJobCatalogPrices } from "./video-jobs.js";

import type { CatalogOperationV1 } from "@betarouter/catalog";
import type { PlatformCatalogOperationV1 } from "@betarouter/db";

// Phase 7 videos-slice exit gate: an operator activates a static video
// mapping through the admission gates — credential, a passed minimal-videos
// probe (a chat probe must NOT satisfy it), a price policy, and enablement —
// the catalog request policy then serves it, and the worker's async billing
// replays the dispatch revision's customer prices (per-second by
// resolution, per-image input, flat request).
const CANARY_MODEL_ID = "grok-imagine-video-1-5";
const CANARY_PROVIDER_ID = "xai";
const ACTOR = "phase7-videos";
...
async function applyOperations(operations: CatalogOperationV1[]) {
	...
	const [changeSet] = await db
		...
		.values({
			createdBy: ACTOR,
			title: "Phase 7 videos activation test",
			reason: "Activate a video mapping through the admission gates",
			state: "draft",
...
describe("catalog videos activation", () => {
	beforeEach(async () => {
		resetCatalogRevisionSnapshotCache();
		await cleanTables();
	});
	...
	it("activates a video mapping only through the minimal-videos probe and replays its prices", async () => {
		...
		const [modelRow] = await db
			...
			.limit(1);
		expect(modelRow?.output).toEqual(["video"]);
		...
		// A passed CHAT probe on the video mapping must not satisfy the test
		...
		// The minimal-videos probe with the same fingerprint-bound target is
		...
		// Video pricing units flow through the price policy: per-second prices
		// stay flat USD (never per-million), per-image input mirrors per
		// million, and the flat request price stays flat.
		expect(effective?.sourcePrices["second:480p"]).toBe("0.08");
		expect(effective?.customerPrices["second:480p"]).toBe("0.08");
		expect(effective?.customerPrices["second:720p"]).toBe("0.14");
		expect(effective?.sourcePrices.imageInput).toBe("10000");
		...
		// Worker billing replay: a job stamped with this revision + mapping
		// bills at the activated customer prices — per-second flat by
		// resolution, image input back at per-image USD, request flat.
		const replay = await getVideoJobCatalogPrices({
			id: "video-job-test",
			catalogRevisionId: applied.id,
		...
		// Jobs without catalog lineage (pre-enforcement dispatch) keep static
		// billing: the replay explicitly reports no catalog prices.
		expect(
			await getVideoJobCatalogPrices({
				id: "video-job-legacy",
				catalogRevisionId: null,
				modelProviderMappingId: null,
			}),
		).toBeNull();
	});
});
