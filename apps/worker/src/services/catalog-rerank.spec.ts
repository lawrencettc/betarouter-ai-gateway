...
// Rerank-slice exit gate: an operator activates a static rerank mapping
// through the admission gates — credential, a passed minimal-rerank probe
// (a chat probe must NOT satisfy it), a price policy, and enablement — and
// the catalog request policy then serves it with the per-token customer
// prices the rerank surface replays synchronously at request time.
const CANARY_MODEL_ID = "qwen3-reranker-0.6b";
const CANARY_PROVIDER_ID = "deepinfra";
const ACTOR = "rerank-slice";
...
async function applyOperations(operations: CatalogOperationV1[]) {
	...
	const [changeSet] = await db
		...
		.values({
			createdBy: ACTOR,
			title: "Rerank slice activation test",
			reason: "Activate a rerank mapping through the admission gates",
			state: "draft",
...
describe("catalog rerank activation", () => {
	beforeEach(async () => {
		resetCatalogRevisionSnapshotCache();
		await cleanTables();
	});
	...
	it("activates a rerank mapping only through the minimal-rerank probe and serves its prices", async () => {
		...
		const [modelRow] = await db
			...
			.limit(1);
		expect(modelRow?.output).toEqual(["rerank"]);
		...
		// A passed CHAT probe on the rerank mapping must not satisfy the test
		...
		// The minimal-rerank probe with the same fingerprint-bound target is
		...
		// Rerank bills on ordinary input tokens: the static 0.01e-6
		// USD/token mapping surfaces as the per-million input unit ($0.01/M),
		// and the rerank surface multiplies the per-token price by the
		// upstream's reported usage at request time.
		expect(Number(effective?.sourcePrices.input)).toBeCloseTo(0.01);
		expect(Number(effective?.customerPrices.input)).toBeCloseTo(0.01);
