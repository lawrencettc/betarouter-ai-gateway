...
```markdown
<img width="1440" alt="<what it shows>" src="https://raw.githubusercontent.com/lawrencettc/betarouter-ai-gateway/<COMMIT>/shot.png" />
```
...
- Verify retention with the Git object API, which is not CDN-fronted — a `200` from `raw.githubusercontent.com` alone proves nothing:
  ...
  ```bash
  gh api repos/lawrencettc/betarouter-ai-gateway/git/blobs/<BLOB>   # .size on success
  ```
